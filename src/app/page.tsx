...
export default function LandingPage() {
  ...
  React.useEffect(() => {
    const saved = localStorage.getItem("hero_email");
            ...
            <div className="space-y-6">
              ...
              >
                <Input
                  ...
                  value={email}
onChange={(e) => { setEmail(e.target.value); localStorage.setItem("hero_email", e.target.value); }}
                  className="bg-background border-border/60"
                />
                <Button className="font-semibold gap-1 flex-shrink-0" onClick={() => {
                    if (email.trim()) {
                      localStorage.setItem("hero_email", email.trim());
                      sessionStorage.setItem("hero_email", email.trim());
                    }
